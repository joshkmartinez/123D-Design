...
// Base texture.
texture gDraftTex : DraftTexture
<
    string UIName = "Draft Texture";
> = NULL;

// Base texture sampler.
sampler2D gDraftSamp : DraftTextureSampler = sampler_state
{
    Texture = <gDraftTex>;
};
...
float4 draftPS(VS_TO_PS In) : COLOR
{
    ...
    UV.x = (vCos + 1) / 2; //Use the vCos as the texture coordinates

    float3 draftColor = tex2D(gDraftSamp, UV).xyz;
